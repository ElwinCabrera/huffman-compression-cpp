...
uint8_t* HuffmanCompress::compress(){
    ...
    this->serialized_huff_code_bit_len = this->serialized_huff_codes.size() * BYTE_LEN;
    ...
    size_t total_bytes = this->serialized_huff_codes.size() + this->compressed_str_bitsequence.get_bytes_allocated();
    ...
    memcpy(tmp_ptr, this->serialized_huff_codes.data(), this->serialized_huff_codes.size());
    tmp_ptr += this->serialized_huff_codes.size();
...
string HuffmanCompress::decompress(uint8_t *data){
    string res = "";
...
string HuffmanCompress::serialize_huff_codes(){
    //unfourunatly will have to serialize as string
    // <char><bit_length><bit#5><bit#4><bit# ..>
    string res = "";
    for(auto& [ch, code]: this->huff_codes){
        res += ch;
        res += to_string(code.get_num_bits());
        int idx = code.get_num_bits() - 1;
        while(idx >= 0){
            if(code.get_bit(idx--)){
                res += "1";
            } else {
...
unordered_map<char, BitSequence> HuffmanCompress::deserialize_huff_codes(uint8_t *data){
    ...
    uint8_t huff_code_len = *data++;
    while(huff_code_len > 0) {
        char ch = *data++;
        uint8_t seq_len = *data++;
        BitSequence bit_seq(seq_len);
        uint64_t idx = seq_len - 1;
        while(seq_len > 0){
            if(*data == '1') bit_seq.set_bit(idx--, true);
            if(*data == '0') bit_seq.set_bit(idx--, false);
            ++data;
            --seq_len;
        }
        res[ch] = bit_seq;
